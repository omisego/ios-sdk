...
class BalanceTests: XCTestCase {
    ...
    func testStandardDisplayAmount() {
        ...
        let balance = StubGenerator.balance(token: token, amount: BigInt("13"))
        XCTAssertEqual(balance.displayAmount(), "0\(decimalSeparator)013")
    }

    func testZeroDisplayAmount() {
        let token = StubGenerator.token(subUnitToUnit: 1000)
        let balance = StubGenerator.balance(token: token, amount: BigInt("0"))
        XCTAssertEqual(balance.displayAmount(), "0")
    }

    func testBigDisplayAmount() {
        let token = StubGenerator.token(subUnitToUnit: 1000)
        let balance = StubGenerator.balance(token: token, amount: BigInt("999999999999999"))
    ...
    func testSmallestDisplayAmount() {
        let token = StubGenerator.token(subUnitToUnit: 1000000000000000000)
        let balance = StubGenerator.balance(token: token, amount: BigInt("1"))
    ...
    func testEquatable() {
        ...
        let balance1 = StubGenerator.balance(token: token1, amount: BigInt("1"))
        let balance2 = StubGenerator.balance(token: token1, amount: BigInt("1"))
        let balance3 = StubGenerator.balance(token: token1, amount: BigInt("10"))
        let balance4 = StubGenerator.balance(token: token2, amount: BigInt("10"))
