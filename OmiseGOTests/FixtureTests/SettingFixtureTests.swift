    ...
    func testGetSettings() {
        ...
        let request = Setting.get(using: self.testCustomClient) { (result) in
            ...
            case .success(let setting):
                ...
                XCTAssertEqual(setting.mintedTokens[0].metadata.count, 0)
                XCTAssertEqual(setting.mintedTokens[0].encryptedMetadata.count, 0)
                ...
                XCTAssertEqual(setting.mintedTokens[1].metadata.count, 0)
                XCTAssertEqual(setting.mintedTokens[1].encryptedMetadata.count, 0)
